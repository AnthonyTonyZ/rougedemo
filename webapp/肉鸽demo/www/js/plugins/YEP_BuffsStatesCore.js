...
/*:zh
 * @plugindesc v1.10 改变在RPG Maker编辑器中无法调整的buff和状态背后的基本机制
 * @author zgn
 *
 * @param ---回合信息---
 * @default
 ...
 *
 * @param Turn Buffer X
 * @desc 将buff回合树的x位置缓冲这么多.
 * @default -3
 *
 * @param Turn Buffer Y
 * @desc 将buff回合树的y位置缓冲这么多.
 * @default -6
 *
 * @param State Color
 * @desc 用于状态回合的默认文本颜色
 ...
 *
 * @param Default Limit
 * @desc 可以堆叠buff/debuff的默认次数.
 * Default: 2
 * @default 4
 *
 * @param Maximum Limit
 * @desc 可以堆叠buff/debuff的最大次数.
 * @default 8
 *
 * @param Buff Formula
 * @desc 用于计算buff命中率的公式.
 ...
 * @desc If using Battle Engine Core, 在帮助窗口显示回合数?
 * NO - false     YES - true
 * @default true
 *
 * @param ---计数 Settings---
 ...
 * ============================================================================
 *
 * 如果您正在使用YEP_BattleEngineCore.js，请在插件列表中的
 * YEP_Battle EngineCore下使用此插件以获得最佳效果
 *
 * 改变在RPG Maker编辑器中无法调整的buff和状态背后的基本机制。这些机制包括
 * 改变buff可以堆叠的最大次数，改变buff和状态的剩余回合数，
 * 以及重新应用状态时所涉及的规则。
 *
 * 增加了一个回合指示器来显示buff、debuff和state的剩余回合数。Buff和
 * debuffs将在插件参数设置下操作，而如果您选择具有唯一设置
 * ，则状态可以在单个设置上操作
 *
 * 此外，该插件还添加了许多Lunatic Mode选项，使那些精通JavaScript的人
 * 能够最大限度地控制状态和随后的任何独特效果。
 *
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 *
 * 以下是可用于修改状态和缓冲区的各种注释标记。
 *
 * --- Buff 相关 ---
 *
 * 角色, 职业, 敌人, 武器, 护甲, 和 状态 注释标记:
 *   <Max stat Buff: +x>
 *   <Max stat Buff: -x>
 *   <Max stat Debuff: +x>
 *   <Max stat Debuff: -x>
 *   将“stat”替换为不带引号的“maxhp”、“maxmp”、“atk”、“def”
 * 、“mat”、“mdf”、“agi”或“luk”。
 * 此注释标记将属性的最大缓冲或减持次数调整为插件参数中的最大限制上限
 *
 * 技能 and 物品 Notetags:
 *   <stat Buff Turns: +x>
 *   <stat Buff Turns: -x>
 ...
 *   <Show Turns>
 *   <Hide Turns>
 *   显示隐藏该状态的剩余回合数。这将覆盖默认设置。.
 *
 *   <Turn Font Size: x>
 *   将用于此特定状态的字体大小设置为x。这将覆盖默认设置
 *
 *   <Turn Alignment: Left>
 *   <Turn Alignment: Center>
 *   <Turn Alignment: Right>
 *   他的设置了回合计数指示器的文本对齐方式。这将覆盖默认设置。
 *
 *   <Turn Buffer X: +x>
 *   <Turn Buffer X: -x>
 *   <Turn Buffer Y: +x>
 *   <Turn Buffer Y: -x>
 *   允许您手动调整该特定状态的回合计数的xy位置。这将覆盖默认设置。
 *
 *   <Turn Color: x>
 *   文本颜色
 *
 ...
 *   <Reapply Reset Turns>
 *   <Reapply Add Turns>
 *  状态{Ignore:不变;Reset:重置默认;Add:叠加}
 *
 * 技能 and 物品 Notetags:
 *   <State x Turns: +y>
 *   <State x Turns: -y>
 ...
 *   <Show State Turns>
 *   <Hide State Turns>
 *   需要Battle Engine Core.
 *  点击敌人在帮助窗口显示状态回合数,需要YEP_BattleEngineCore.js
 *
 * ============================================================================
 * Lunatic Mode - Custom Turn Modifiers
 * ============================================================================
 *
 * 对于那些了解JavaScript的人来说，当改变目标的buffstate回合数时，
 * 可以使用这些注释标签来提供条件回合修饰符。请遵循以下说明:
 *
 * 技能 and 物品 Notetags:
 *
 ...
 * ============================================================================
 *
 * 对于那些了解JavaScript的人来说，
 * 可以使用这些注释标记使代码在特定时间运行。
 *
 * Quick Reference:
 *   - Apply:        添加.
 *   - Remove:       移除.
