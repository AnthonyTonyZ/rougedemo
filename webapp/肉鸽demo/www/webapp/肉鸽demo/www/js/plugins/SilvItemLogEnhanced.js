...
//=============================================================================
/*:zh
 * @plugindesc 用彩色显示上次掠夺的物品，并添加弹窗提示
 ...
 * @desc 是否自动将战利品添加到派对的库存中? true/false
 * @default true
 *
 * @param Auto Log Items
 * @desc 通过GainItem命令添加项目时，自动将项目添加到日志窗口? true/false
 * @default true
 *
 * @param -- Sound --
 *
 ...
 *
 * @param Default Pitch
 * @desc 所有ItemLog的默认间距 SFX
 * @default 100
 *
 * @param Item SFX
 * @desc 取物品、盔甲或武器时自动播放此声音。留空可禁用。.
 * @default Item1
 *
 * @param Gold SFX
 * @desc 拾取黄金（货币）时自动播放此声音。保留为空以禁用.
 * @default Coin
 *
 * @param -- Positioning & Size --
 *
 * @param Window X
 * @desc 项目日志窗口的x位置。如果窗口对齐设置为“右”，则此值将作为偏移值
 * @default -2
 *
 * @param Window Y
 * @desc y
 ...
 * @desc 如果项目窗口完全褪色，则清除该窗口中的项目? true/false
 * @default false
 *
 * @param -- Miscellaneous --
 *
 * @param Fadeout Delay
 * @desc 窗口开始淡出前的时间（以帧为单位）
 * @default 240
 *
 * @param Fadeout Speed
 * @desc 窗口淡出的速度
 ...
 * @desc 编写自定义提示模板. Tags:{$prefixtext},{$amount},{$name},{$icon},{$unitprice},{$totalprice},{$description},\G,\C,\N,\F...
 ...
 * @desc 通过ChangeSwitches命令添加开关时，自动将开关添加到日志窗口? true/false
 * @default true
 */
/*:
